$(function () {
    (function () {
        ...
        //加载工具栏，并且隐藏之前页面定义的按钮
        $("#dinner").jqGrid('navGrid', '#dinnerPager', {
            edit: false,
            add: false,
            del: false,
            search: true
        ...
        // 修改按钮
        $("#dinner").navButtonAdd('#dinnerPager',
        {
            caption: "修改",
            buttonicon: "glyphicon glyphicon-edit",
            onClickButton: function(){
                // 获取页面上多选框选中之后返回的id数组
                var ids = $("#dinner").jqGrid("getGridParam","selarrrow");
                // 调用修改的方法
                openEditModal(ids);
            },
            position: "first"
        ...
        function get_jqgrid () {
            async('../Dinner/jqgrid_all','post','',function (data) {
                if (data.code == 0) {
                    jq_data = data.message;
                    $("#dinner").jqGrid('clearGridData');  //清空表格
                    $("#dinner").jqGrid('setGridParam',{  // 重新加载数据
                        datatype:'local',
                        data :jq_data, 
                    }).trigger("reloadGrid");
                }            
        ...
        function openEditModal (ids) {
            ...
            if (idsLength == 0) {
                ...
                //获取某一行的数据，并且进行赋值
                var getIdRow = $("#dinner").jqGrid("getRowData",ids);
                ...
                $('#editForm').validate({
                    ...
                    submitHandler:function () {
                        ...
                        // 初始化楼层下拉框内容
                        async('../Dinner/update','post',formData,function (data) {
            ...
            */
            if(idsLength<1){
                ...
                swal({
                    ...
                    console.log(idStr);
                    async('../Dinner/delete','post',{ids:idStr},function (data) {
                        var message = data.message;
                        if (data.code == 0) {
        ...
        function openAddModal () {
            ...
            $('#addForm').validate({
                ...
                submitHandler:function () {
                    ...
                    console.log(formData);
                    async('../Dinner/add','post',formData,function (data) {
                        var message = data.message;
                        if (data.code == 0) {
                            swal({
                                title: "",
